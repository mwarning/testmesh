...
static void handle_RREQ(const Address *addr, RREQ *p, size_t recv_len)
{
    ...
    routing_table_update(p->src_id, addr, p->hop_count, &p->bloom[0], p->seq_num);
...
static void handle_RREP(const Address *addr, RREP *p, size_t recv_len)
{
    ...
    log_debug("RREP: got packet: %s / 0x%08x (0x%08x) => 0x%08x / hop_count: %u, seq_num: %u",
        str_addr(addr), p->origin_id, p->src_id, p->dst_id, p->hop_count, p->seq_num);

    routing_table_update(p->origin_id, addr, p->hop_count, NULL, p->seq_num);

    if (p->dst_id == gstate.own_id) {
    	log_debug("RREP: reached destiantion => send cached packet");
...
static void handle_DATA(const Address *addr, DATA *p, size_t recv_len)
{
    ...
    routing_table_update(p->src_id, addr, p->hop_count, NULL, p->seq_num);
